...
export const hidePost = createAsyncThunk(
  'posts/hidePost',
  async (
    { postId, hidden }: { postId: string; hidden: boolean },
    { dispatch },
  ) => {
    const response = await axiosInstance.patch(`/posts/${postId}/visibility`, {
      hidden,
    });
    dispatch(
      addToast({
...
export const hideComment = createAsyncThunk(
  'posts/hideComment',
  async (
    { commentId, hidden }: { commentId: string; hidden: boolean },
    { dispatch },
  ) => {
    ...
    dispatch(
      addToast({
        message: `Comment ${hidden ? 'hidden' : 'shown'} successfully`,
        type: 'success',
      }),
    );
    return response.data;
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<PaginatedResponse<Post>>) => {
          state.loading = false;
      ...
      .addCase(hidePost.rejected, (state, action) => {
        state.error =
          action.error.message || 'Failed to update post visibility';
      })
      ...
      .addCase(hideComment.rejected, (state, action) => {
        state.error =
          action.error.message || 'Failed to update comment visibility';
      });
  },
});
